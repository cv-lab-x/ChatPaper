...
class PDFFigure2PaperParser(PDFPaperParser):
    ...
    def get_section_imagedict(self, snapWithCap=SNAP_WITH_CAPTION, verbose=False):
        ...
        """
        img_ls = []
        # print("PDFF2data:", self.PDFF2data)
        img_figures = []
        img_tables = []
        for d in self.PDFF2data['figures']:
            print( d['page'], d['figType'], d['name'])
            if snapWithCap:
                ...
                img_ls.append((get_box_textpos(self.pdf[d['page']], box, self.all_text),
                               d['page'], box, d['figType'], d['name']))
                if d['figType'] == "Figure":
                    img_figures.append((get_box_textpos(self.pdf[d['page']], box, self.all_text),
                               d['page'], box, d['figType'], d['name']))
                elif d['figType'] == "Table":
                    img_tables.append((get_box_textpos(self.pdf[d['page']], box, self.all_text),
                                d['page'], box, d['figType'], d['name']))
            else:
        ...
        if verbose:
            print('Images match the content:', match_cnt)
        return section_dict, img_figures, img_tables
